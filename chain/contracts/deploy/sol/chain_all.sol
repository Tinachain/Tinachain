...
contract safeMath {
    ...
    function safeMul(uint a, uint b) internal returns (uint256) {
        uint c = a * b;
        assert(a == 0 || c / a == b);
        return c;
    }
    ...
    function safeSub(uint a, uint b) internal returns (uint256) {
        assert(b <= a);
        return a - b;
    }
...
contract CWAREToken is owned, TokenERC20 {
            /* Public variables of the token */
    ...
    */
    ...
    struct Task {
        uint256 index;
        uint256 value;      //Lock Token
        string taskIp;      //IP Address for Submitting Task
    }
    ...
    function lockToken(address target, bytes32 taskId, uint256 lockedAmount, string taskIp) public 
            returns(bool) {
    ...
    function unLockToken(address target, bytes32 taskId) public 
            returns(bool) {
...
////////////////////////////////////////
////////////////////////////////////////
contract Chainware is owned, safeMath{
...
//    function getNodeFromList(nodeSet ns);

    //constructor
    function Chainware(address cware_token) 
       owned(msg.sender) {
        numTaskOrder = 0;
        numComputeNode = 0;
    ...
    //define events
    event ClientEvent(bytes32 _id, address indexed _client, uint64 _indexID, address indexed _computeNode, 
                      uint _taskGas, uint _createTime, uint32 _state, string _reason);
    event NodeEvent(bytes32 _id, address indexed _node, uint _hashRate, uint _enrollTime, string _reason);
    event AssignEvent(bytes32 _id, address indexed _client, uint64 _index, address indexed _computeNode, 
                      uint _taskGas, uint _assignTime, string _codeHash);
    event CompleteEvent(bytes32 _id, address _client, uint64 _index, address _computeNode, 
                      uint _completeTime, string _resultHash);
    event LogEvent(bytes32 _id, address indexed _client, address indexed _computeNode, 
                      string _reason);
    ...
    function taskOrderRequest(uint64 index, uint256 taskGas, string codeHash, string taskIp) 
        ...
        if ((o.state != taskState.Unrequested) && (o.client == msg.sender)) {
            ...
            ClientEvent(o.orderID, o.client, o.index,  o.computeNode, o.taskGas,
                        o.createTime, o._state, "task already existed, cancel first");
            return false;
        }
        ...
        if (CWAREToken(CWARE_Token_Addr).lockToken(msg.sender, taskId, taskGas, taskIp) != true) {
            ClientEvent(orderID, msg.sender, index, 0, taskGas,
                        0, 0, "Lock token failed");
            return false;
        }
        ...
        ClientEvent(o.orderID, o.client, o.index, o.computeNode, o.taskGas, 
                    o.createTime, o._state, "order request confirmed");
        return true;
    }

    // Cancel
    /// @notice Cancel `taskOrderCancel` cancel task order from client
    function taskOrderCancel(bytes32 orderID) 
        public returns (bool) { //send out the request
        var o = orderBook[orderID];
        ...
        if (o.client != 0) {
            ClientEvent(o.orderID, o.client, o.index, o.computeNode, o.taskGas, 
                        o.createTime, o._state, "order canceled");   
        }
    ...
    function computingNodeEnroll(uint64 hashRate)
        public returns (bool) {
    ...
    function computingNodeDisconnect (address node)
        public returns (bool) {
        var n = nodeSets[node];
        if (n.enrolled != true) {
    ...
    function computingTaskComplete(address node, bytes32 orderID, 
                                   uint256 realGas, string resultHash) 
    //TBD: actual HashRate, Proof of code execution are needed
        public returns (bool) {
        var o = orderBook[orderID];
        ...
        var payMinerAmount = safeDiv(taskGas, 5);
...
*****/
    ...
    function assignTaskOrder(taskOrder storage o) 
        ...
        for (uint32 i = 0; i <= MAX_SEARCH_ASSIGNED_NODE; i++) {
            ...
            if ((ns.enrolled == true) && (ns.assignedClient == 0)) {
                ...
                uint completeTime = 
                    safeAdd(safeDiv(o.taskGas, ns.hashRate), PREPARE_TIME);
                ...
                if (completeTime <= MAX_WAITING_TIME) {
                    ...
                    o.state = taskState.Assigned;
                    o._state = 2;
                    AssignEvent(o.orderID, o.client, o.index, nodeAddr, o.taskGas, 
                                now, o.info[0].codeHash);
    ...
    function getOneTaskOrder(bytes32 orderID) 
        public view returns (bool) {
        for (uint i=0; i<taskOrderList.length; i ++) {
            if (taskOrderList[i] == orderID) {
                var o = orderBook[orderID];
                ClientEvent(o.orderID, o.client, o.index, o.computeNode, o.taskGas, 
                            o.createTime, o._state, "query order");
                break;
            }
        }
        return (true);
    ...
    //query all task node
    /// @notice Create `taskOrderRequest` create task order for request
    function getAllTaskOrder() 
        public returns (bool) {

        for (uint i=0; i<taskOrderList.length; i++) {
            var o = orderBook[taskOrderList[i]];
            ClientEvent(o.orderID, o.client, o.index, o.computeNode, o.taskGas, 
                        o.createTime, o._state, "query oder");
        }
    ...
    /// @notice Create `taskOrderRequest` create task order for request
    function houseHoldAllTaskOrder() 
        public returns (bool) {
        for (uint i=0; i<taskOrderList.length; i++) {
            var o = orderBook[taskOrderList[i]];
