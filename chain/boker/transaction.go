...
func (t *BokerTransaction) SubmitBokerTransaction2(ctx context.Context,
	txMajor protocol.TxMajor,
	txMinor protocol.TxMinor,
	from common.Address,
	to common.Address,
	...
	if t.ethereum != nil {
		...
		wallet, err := t.ethereum.AccountManager().Find(account)
		if err != nil {
			log.Error("SubmitBokerTransaction2 AccountManager Find", "error", err)
			return nil, err
		}

		//设置默认设置
		if err := args.SetDefaults(ctx, t.ethereum.ApiBackend); err != nil {
			log.Error("SubmitBokerTransaction2 SetDefaults", "error", err)
			return nil, err
		}
		log.Info("SubmitBokerTransaction2 SetDefaults",
			"Nonce", args.Nonce.String(),
			"Major", args.Major,
			"Minor", args.Minor,
			"Name", args.Name,
		...
		if protocol.SystemBase == txMajor || protocol.UserBase == txMajor {
			...
			log.Info("SubmitBokerTransaction2 tx", "Major", tx.Major(), "Miner", tx.Minor(), "Extra", tx.Extra())

		} else {

			log.Error("SubmitBokerTransaction2 txType Not Found")
			return nil, errors.New("SubmitBokerTransaction2 txType Not Found")
		}

		log.Info("SubmitBokerTransaction2 NewTransaction", "Value", tx.Value, "tx.Hash", tx.Hash().String())

		//对该笔交易签名来确保该笔交易的真实有效性
		signed, err := wallet.SignTxWithPassphrase(account, t.ethereum.Password(), tx, chainID)
		if err != nil {

			log.Error("SubmitBokerTransaction2 SignTxWithPassphrase", "error", err)
			return nil, err
		}

		log.Info("SubmitBokerTransaction2 SignTxWithPassphrase",
			"Gas", signed.Gas(),
			"GasPrice", signed.GasPrice(),
			"Value", signed.Value,
			"Nonce", signed.Nonce(),
		...
		if _, err := ethapi.SubmitTransaction(ctx, t.ethereum.ApiBackend, signed); err != nil {
			...
			log.Error("SubmitBokerTransaction2 SubmitTransaction", "error", err)
			return nil, err
		}
