...
type OnStreamStatistics func(stream *Stream, statistics *StreamStatistics)
...
type StreamCallback struct {
	...
	OnStatistics       OnStreamStatistics
}

type StreamStatistics struct {
	M3u8DownloadCount int64
}
...
func (stream *Stream) onStatistics(statistics *StreamStatistics) {
	if stream.Callback.OnStatistics != nil {
		go stream.Callback.OnStatistics(stream, statistics)
	}
}
...
func (stream *Stream) DownloadM3u8() (m3u8 *M3u8, err error) {
	m3u8String := ""
	atomic.AddInt64(&stream.M3u8DownloadCount, 1)

	if stream.AntileechRemote != "" {
		headers := http.Header{}
		headers.Set("Strm-Uri", stream.M3u8Url)
		antileechUrl := antileech.AntileechUrl(stream.AntileechRemote)
		m3u8String, err = httputils.HttpGet(antileechUrl, stream.Timeout, headers)
	} else {
...
func (stream *Stream) pull() {
	...
	for {
		...
		stream.onStatistics(&StreamStatistics{M3u8DownloadCount: stream.M3u8DownloadCount})
