...
func getYear(header *types.Header) (*big.Int, *big.Int) {
	...
	//根据年份判断分币情况
	if year <= 2022 {
		return new(big.Int).SetUint64(6341958), new(big.Int).SetUint64(25367833)
	} else if year > 2022 && year <= 2026 {
		return new(big.Int).SetUint64(3170979), new(big.Int).SetUint64(12683916)
	} else if year > 2026 && year <= 2030 {
		return new(big.Int).SetUint64(1585489), new(big.Int).SetUint64(6341958)
	} else {
		return new(big.Int).SetUint64(1585489), new(big.Int).SetUint64(6341958)
	}
}
...
func AccumulateRewards(config *params.ChainConfig,
	...
	//根据块时间判断当前属于哪一个分币档次
	tinaMultiple, userMultiple := getYear(header)
